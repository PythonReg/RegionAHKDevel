...
Gui, 2: Add, Text, x0 y0 w900 h45 +Center +0x200 Background23272a, REGION HELPER ; Темная полоса сверху
Gui, 2: Font, s8 Norm c4f545c
Gui, 2: Add, Text, x840 y12 w50 h20 +BackgroundTrans +Right, %Version%

; --- БОКОВОЕ МЕНЮ (Sidebar) ---
; Вместо GroupBox используем стильный разделитель
...
Gui, 2: Font, s1 Norm c7289da
Gui, 2: Add, Text, x610 y82 w275 h2 +Border
...
Gui, 2: Font, s9 Norm cFFFFFF
Gui, 2: Add, DropDownList, x610 y205 w275 vScreenRes, 1920x1080||1680x1050|1600x900|1440x900|1366x768|1280x1024|1280x960|2560х1440
...
::.тп::
; Настройки задержки для имитации печати человека (40мс между нажатиями, 40мс удерживание)
SetKeyDelay, 40, 60

targetX := 0, targetY := 0, lang := 0x4090409, extraColors := false

if (Radio1 == 1) {
    targetX := 1410, targetY := 187
    extraColors := (Var1 = 0x008313FF || Var1 = 0x006619E9)
} else if (Radio2 == 1) {
    ...
    targetX := 1263, targetY := 193
    lang := 0x4190419 
} else if (Radio20 == 1) {
    targetX := 901, targetY := 182
    lang := 0x4190419 
} else if (Radio16 == 1) {
...
isSpecialColor := (Var1 = 0x005A1CE8 || Var1 = 0x005912FB || extraColors)

; Установка языка
SendMessage, 0x50,, %lang%,, A

if (isSpecialColor) {
    ; Используем SendEvent вместо SendInput для соблюдения задержек SetKeyDelay
    text := (Radio1 == 1) ? "Приветствую. Сейчас помогу Вам.{Enter}" : "Приветствую. Уже лечу к вам.{Enter}"
    SendEvent, %text%
    
    Sleep 600 ; Пауза побольше, чтобы чат закрылся
    GoSub, SendTPAction
    ...
    SendEvent, Приветствую. Уже лечу к вам.{Enter}
    Sleep 600 
    GoSub, SendTPAction
}
Return
...
SendTPAction:
    ; Переключаем на английский для команды
    SendMessage, 0x50,, 0x4090409,, A
    Sleep 150
    
    ; Открываем чат
    SendEvent, {T}
    Sleep 300 ; Ждем открытия строки ввода
    
    ; Печатаем команду по буквам
    SendEvent, /tp{space}
return
