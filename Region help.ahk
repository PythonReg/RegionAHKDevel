...
{
    FileAppend, /jail 0 60 Причина`n, %PunishPath%, UTF-8
}
...
Gui, 2: Font, s8 Bold c7289da
Gui, 2: Add, Text, x20 y305 w120 h20, ФАЙЛЫ
Gui, 2: Font, s9 Norm cFFFFFF
Gui, 2: Add, Button, x15 y325 w130 h45 gOpenPunishmentFile, Редактировать`nнаказания
...
; Первый столбец
y_p := 95
; Массив: Название | Номер переменной
...
Gui, 7: Add, Text, x20 y225 w180 h20, Иду на помощь:
Gui, 7: Add, Hotkey, x220 y222 w155 h24 vNewKeyGo, %KeyGo%
Gui, 7: Add, Text, x20 y260 w180 h20, Ожидайте помощь:
Gui, 7: Add, Hotkey, x220 y257 w155 h24 vNewKeyWait, %KeyWait%
Gui, 7: Add, Text, x20 y295 w180 h20, Приятной игры:
Gui, 7: Add, Hotkey, x220 y292 w155 h24 vNewKeyPlay, %KeyPlay%

; --- ВЫДАЧА НАКАЗАНИЙ (НОВОЕ) ---
Gui, 7: Font, s8 Bold c7289da
Gui, 7: Add, Text, x20 y335, [ МАССОВАЯ ВЫДАЧА НАКАЗАНИЙ ]
Gui, 7: Font, s9 Norm cWhite
Gui, 7: Add, Text, x20 y365 w180 h20, Выдать из файла:
Gui, 7: Add, Hotkey, x220 y362 w155 h24 vNewKEY13, %KEY13%
Gui, 7: Font, s8 cBDC3C7
Gui, 7: Add, Text, x30 y387 w350 h15, » Считать Punishment.txt и выдать в чат

; Кнопка
Gui, 7: Font, s10 Bold c1A1A1A
Gui, 7: Add, Button, x20 y420 w360 h40 gSaveSystemKeys, СОХРАНИТЬ И ПРИМЕНИТЬ
Gui, 7: Show, w400 h480, Настройка биндов
return
...
return
PunishmentHandler:
Process, Exist, GTA5.exe ; Чаще процесс игры называется так
if (Errorlevel)
{
    ...
    Loop, read, %A_WorkingDir%\Punishment.txt
    {
        ...
        Loop, parse, A_LoopReadLine, %A_Tab%
        {
            ...
            SendInput, {Enter}
            Sleep, 1000 ; Задержка чуть больше, чтобы антифлуд не кикнул
        }
    }
...
{
    MsgBox, 16, Ошибка, Игра не найдена! Убедитесь, что вы в игре.
}
return
gcar:
BlockInput, SendAndMouse
